...
void PID_init( float P_set, float D_set, float I_set, float I_Limit_Set, float MaxOutput_Set, float DeadBand_Set)
{
  int i = 0;
  for(i = 0;i < 4;i++)
  {
    PIDControl_FunctionDefine(&WheelControl[i]);
    PIDControl_Constructor(&WheelControl[i], P_set, D_set, I_set, I_Limit_Set, MaxOutput_Set, DeadBand_Set);
  }
