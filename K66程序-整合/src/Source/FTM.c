...
void FTM_PWM_Init(FTM_Type * ftmn, FTM_CHn_e ch, u32 freq, u32 duty)
{
  ...
  /******************* 开启时钟 和 复用IO口*******************/
   ...
   case FTM3_BASE:
    ...
    switch ((u32)ftmn)                // 初值 CNTIN 设为0 ，脉冲宽度：CnV - CNTIN ，即 CnV 就是 脉冲宽度了。
    {
    ...
    /******************** 配置时钟和分频 ********************/
    FTM_SC_REG(ftmn)    = ( 0
                                  | FTM_SC_PS(ps)      //分频2^FTM_SC_PS,频率为 40M/8=5M
