...
int16 MT9V032_CFG_1[CONFIG_FINISH][2]=
{
    ...
    {FPS,               100},  //50图像帧率          摄像头收到后会自动计算出最大FPS，如果过大则设置为计算出来的最大FPS
...
int16 MT9V032_CFG_2[CONFIG_FINISH][2] =
{
    { AUTO_EXP, 0 },   //自动曝光设置      范围1-63 0为关闭 如果自动曝光开启  EXP_TIME命令设置的数据将会变为最大曝光时间，也就是自动曝光时间的上限
    //一般情况是不需要开启这个功能，因为比赛场地光线一般都比较均匀，如果遇到光线非常不均匀的情况可以尝试设置该值，增加图像稳定性
    { EXP_TIME, 40 },//3118, //曝光时间          摄像头收到后会自动计算出最大曝光时间，如果设置过大则设置为计算出来的最大曝光值
    { FPS, 200 },  //50图像帧率          摄像头收到后会自动计算出最大FPS，如果过大则设置为计算出来的最大FPS
    { SET_COL, COL }, //图像列数量        范围1-752     K60采集不允许超过188
    { SET_ROW, ROW }, //图像行数量        范围1-480
    { LR_OFFSET, 0 },   //图像左右偏移量    正值 右偏移   负值 左偏移  列为188 376 752时无法设置偏移    摄像头收偏移数据后会自动计算最大偏移，如果超出则设置计算出来的最大偏移
    { UD_OFFSET, 0 },   //图像上下偏移量    正值 上偏移   负值 下偏移  行为120 240 480时无法设置偏移    摄像头收偏移数据后会自动计算最大偏移，如果超出则设置计算出来的最大偏移
    { GAIN, 0 },  //32 图像增益          范围16-64     增益可以在曝光时间固定的情况下改变图像亮暗程度
...
uint16 get_version(void)
{
    ...
    uart_putbuff(MT9V032_COF_UART_1,send_buffer,4);
...
void camera_init_1(void)
{
    ...
    UART_Init(MT9V032_COF_UART_1, 9600);	//初始换串口 配置摄像头    
    UART_Irq_En(MT9V032_COF_UART_1);
    ...
    EXTI_Init(MT9V032_VSYNC_1, MT9V032_VSYNC_CHANNEL_1, falling_up);
...
void camera_init_2(void)
{
    ...
    UART_Init(MT9V032_COF_UART_2, 9600);	//初始换串口 配置摄像头    
    ...
    EXTI_Init(MT9V032_VSYNC_2, MT9V032_VSYNC_CHANNEL_2, falling_up);
    set_irq_priority(PORTA_IRQn, 3);
    EnableInterrupts;
}
extern uint32 TimeMeassure;
...
void seekfree_sendimg_032(void)
{
  ...
  UART_Put_Char(UART_4,0x01);
  UART_Put_Buff(UART_4,(uint8 *)image_2,ROW*COL);
